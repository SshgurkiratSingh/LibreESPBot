...
float CompassDriver::getHeading() {
    ...
    // Dynamic auto-calibration for Hard Iron bias
    if (x < _minX) _minX = x;
    if (x > _maxX) _maxX = x;
    if (y < _minY) _minY = y;
    if (y > _maxY) _maxY = y;
    ...
    // Only apply calibration offset if we have seen enough physical rotation 
    // to distinguish the signal from stationary sensor noise.
    ...
    float h = atan2f(calY, calX) * 180.0f / PI;
    if (h < 0.0f) h += 360.0f;
    return h;
}
